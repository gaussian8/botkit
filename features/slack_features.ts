...
module.exports = function (controller: Botkit) {
  controller.ready(async () => {
    if (process.env.MYTEAM) {
      let bot = (await controller.spawn(process.env.MYTEAM)) as SlackBotWorker;
      await bot.startConversationInChannel(process.env.MYCHAN, process.env.MYUSER);
      bot.say("I AM AWOKEN.");
    }
  });

  controller.on("direct_message", async (bot: SlackBotWorker, message) => {
    await bot.reply(message, "I heard a private message");
  });

  controller.hears("dm me", "message", async (bot: SlackBotWorker, message) => {
  ...
  controller.hears("ephemeral", "message,direct_message", async (bot: SlackBotWorker, message) => {
    await bot.replyEphemeral(message, "This is an ephemeral reply sent using bot.replyEphemeral()!");
  });

  controller.hears("threaded", "message,direct_message", async (bot: SlackBotWorker, message) => {
  ...
  controller.on("interactive_message", async (bot: SlackBotWorker, message) => {
  ...
  controller.on("dialog_submission", async (bot: SlackBotWorker, message) => {
